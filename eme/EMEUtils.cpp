...
bool
IsPlayReadyKeySystem(const nsAString& aKeySystem)
{
  return !CompareUTF8toUTF16(kEMEKeySystemPlayReady, aKeySystem);
}
...
KeySystemToGMPName(const nsAString& aKeySystem)
{
  ...
  if (IsPlayReadyKeySystem(aKeySystem)) {
    return NS_LITERAL_STRING("gmp-playready");
  }
...
ToCDMTypeTelemetryEnum(const nsString& aKeySystem)
{
  ...
  } else if (IsPlayReadyKeySystem(aKeySystem)) {
      return CDMType::ePlayReady;
  }
