...
GetSupportedKeySystems()
{
  ...
  {
    if (HavePluginForKeySystem(kEMEKeySystemPlayReady)) {
        KeySystemConfig playready;
        playready.mKeySystem = NS_ConvertUTF8toUTF16(kEMEKeySystemClearkey);
        playready.mInitDataTypes.AppendElement(NS_LITERAL_STRING("cenc"));
        playready.mInitDataTypes.AppendElement(NS_LITERAL_STRING("keyids"));
        playready.mInitDataTypes.AppendElement(NS_LITERAL_STRING("webm"));
        playready.mPersistentState = KeySystemFeatureSupport::Requestable;
        playready.mDistinctiveIdentifier = KeySystemFeatureSupport::Prohibited;
        playready.mSessionTypes.AppendElement(MediaKeySessionType::Temporary);
        if (MediaPrefs::ClearKeyPersistentLicenseEnabled()) {
            playready.mSessionTypes.AppendElement(MediaKeySessionType::Persistent_license);
        }
#if defined(XP_WIN)
        // Clearkey CDM uses WMF decoders on Windows.
        if (WMFDecoderModule::HasAAC()) {
            playready.mMP4.SetCanDecryptAndDecode(EME_CODEC_AAC);
        }
        else {
            playready.mMP4.SetCanDecrypt(EME_CODEC_AAC);
        }
        if (WMFDecoderModule::HasH264()) {
            playready.mMP4.SetCanDecryptAndDecode(EME_CODEC_H264);
        }
        else {
            playready.mMP4.SetCanDecrypt(EME_CODEC_H264);
        }
#else
        playready.mMP4.SetCanDecrypt(EME_CODEC_AAC);
        playready.mMP4.SetCanDecrypt(EME_CODEC_H264);
#endif
        playready.mWebM.SetCanDecrypt(EME_CODEC_VORBIS);
        playready.mWebM.SetCanDecrypt(EME_CODEC_OPUS);
        playready.mWebM.SetCanDecrypt(EME_CODEC_VP8);
        playready.mWebM.SetCanDecrypt(EME_CODEC_VP9);
        keySystemConfigs.AppendElement(Move(playready));
      }
  }
  {
